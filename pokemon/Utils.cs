...
public static class PokemonReader
{
    public static Pokemon ReadPokemonFromFile(string path)
    {
        ...
        if (lines.Length >= 9)
        {
            P.movements.Add(MovementDatabase.GetMovement(lines[9]));
        }
        if (lines.Length >= 10)
        {
            P.movements.Add(MovementDatabase.GetMovement(lines[10]));
        }
        if (lines.Length >= 11)
        {
            P.movements.Add(MovementDatabase.GetMovement(lines[11]));
        }
        if (lines.Length >= 12)
        {
            P.movements.Add(MovementDatabase.GetMovement(lines[12]));
        }
